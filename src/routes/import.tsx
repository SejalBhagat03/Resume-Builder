    ...
    try {
      ...
      if (file.name.toLowerCase().endsWith(".pdf")) {
        try {
          ...
          // Backup upload to Supabase Storage if user is authenticated
          try {
            const { data: { session } } = await supabase.auth.getSession();
            if (session?.user) {
              const { data: storageData, error: uploadErr } = await supabase.storage
                .from("imported_resumes")
                .upload(`${r.id}.pdf`, file, { upsert: true });

              if (!uploadErr && storageData) {
                r.data.importedPdf.storagePath = storageData.path;
              } else {
          ...
          setProgressStep(4);
          try {
            const textForAI = rawText || importedLayout.pages?.flatMap((p: any) => p.textItems.map((i: any) => i.text)).join(" ") || "";
            if (textForAI && textForAI.length >= 20) {
              const { parseResumeStructure } = await import("@/lib/ai.functions");
              const parsed = await parseResumeStructure({ data: { resumeText: textForAI } });
          ...
          try {
            ...
            if (rawText.length >= 20) {
              const { parseResumeStructure } = await import("@/lib/ai.functions");
              const parsed = await parseResumeStructure({ data: { resumeText: rawText } });
              r.data.fullName = parsed.fullName || "";
              r.data.email = parsed.email || "";
              r.data.phone = parsed.phone || "";
              r.data.location = parsed.location || "";
        ...
        try {
          if (text.length >= 20) {
            const { parseResumeStructure } = await import("@/lib/ai.functions");
            const parsed = await parseResumeStructure({ data: { resumeText: text } });
            r.data.fullName = parsed.fullName || "";
            r.data.email = parsed.email || "";
            r.data.phone = parsed.phone || "";
            r.data.location = parsed.location || "";
          ...
          const nameLine = lines.find((l) => l.length > 2 && l.length < 60 && !l.includes("@") && !l.includes("http"));
                  ...
                  return (
                    <div key={idx} className="flex items-center justify-between text-xs">
                      ...
                      <span className={cn(
                        ...
                        !isActive && !isDone && "bg-muted text-muted-foreground/15 border border-border"
                      )}>
                        {isDone ? "✓" : isActive ? "●" : ""}
                      </span>
                    </div>
